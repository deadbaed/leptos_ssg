    ...
    view! {
        <footer class=tw_join!("bg-black")>
            <div class=tw_join!("container", "mx-auto", "py-8", "px-4", "sm:px-8", "md:px-16", "lg:px-32", "xl:px-64", "2xl:px-96")>
                <p class=tw_join!("text-white")>"Page generated with "{underline_link("https://github.com/deadbaed/leptos_ssg", "leptos_ssg", None)}" on "{timestamp.to_string()}" "<span data-relative-timestamp={timestamp.as_millisecond()}></span></p>
            </div>
        </footer>
    }
}
        ...
        container(view! {
            <div class=tw_join!("md:flex", "md:space-x-4")>
                <div class=tw_join!("mb-2", "md:mb-0")>
                    <img alt=format!("{} logo", config.website_name) src=format!("{}{}", config.base_url, config.logo) class=tw_join!("w-42", "h-42", "border-1", "bg-gray-400", "dark:bg-gray-600") />
                </div>
                {title_view(page_title, subtitle).into_any()}
            </div>
            {nav_and_content(header, children).into_any()}
